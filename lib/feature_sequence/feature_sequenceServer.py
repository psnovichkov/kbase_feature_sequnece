...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'feature_sequence'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from feature_sequence.feature_sequenceImpl import feature_sequence
impl_feature_sequence = feature_sequence(config)
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "feature_sequence but no authentication header was passed"
                            raise err
